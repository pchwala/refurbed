...
def update_states():
    ...
    try:
        ...
        # Use the RefurbedAPI directly
        api.refurbed_api.update_states()
        
        # Get all orders to calculate totals (this is just for reporting)
        r_URL = "https://api.refurbed.com/refb.merchant.v1.OrderService/ListOrders"
        payload = api.refurbed_api.payload_all()
        response = requests.post(r_URL, headers=api.ref_headers, json=payload)
        response_data = response.json()
        total_count = len(response_data.get('orders', []))
        
        # Get information about which orders were updated
        all_rows = api.orders_sheet.get_all_values()
        header = all_rows[0]
        r_state_col = header.index("r_state") + 1
        id_col = header.index("ID") + 1
        
        # Create a dictionary of row_number -> order details for quick lookup
        updated_orders = []
        for i, row in enumerate(all_rows[1:], start=2):
            if len(row) >= id_col and row[id_col - 1]:
                order_id = row[id_col - 1]
                order_state = row[r_state_col - 1] if len(row) >= r_state_col else ""
                # Collect orders that are in the sheet
        ...
        output = f"Zaktualizowano stany zamówień w arkuszu."
        
        # Show details of orders
        if updated_count > 0:
            output += "\n\nZnaleziono zamówienia:"
            # Show the first 10 orders
            for order in updated_orders[:10]:
                output += f"\nWiersz {order.get('row')}: ID {order.get('id')} - Stan: {order.get('state')}"
            
            # Indicate if there are more orders
            if updated_count > 10:
                output += f"\n\n...oraz {updated_count - 10} więcej zamówień"
